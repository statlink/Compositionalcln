ternary <- function(y, dg = FALSE, hg = FALSE, m = NULL, colour = NULL) {
 ...
 if ( dg ) {
  ...
  points( d[1:n, 1], d[1:n, 2], col = colour, pch = symb, lwd = 2 )
  if ( !is.null(m) ) {
    ## should the means appear in the plot?
    points( d[c(n + 1), 1], d[c(n + 1), 2], pch = 3, col = 3, lwd = 2 )
  }
